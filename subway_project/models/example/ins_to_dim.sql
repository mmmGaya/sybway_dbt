select ma.run_id dataflow_id, ma.execution_date dataflow_dttm, client_rk, valid_from_dttm, valid_to_dttm, 
	   client_name_desc, client_phone_desc, client_city_desc, client_birthday_dt, client_age_cnt
from
(select distinct 
	   ac.client_rk, pc.valid_from_dttm, pc.valid_to_dttm, sc.client_name_desc, sc.client_phone_desc, sc.client_city_desc, sc.client_city_dt client_birthday_dt, sc.client_age_cnt,
...
where extract(year from pc.valid_from_dttm) > 1960 and sc.delete_flg = 0 and sc.actual_flg = 1), (select * from dbt_schema.metadata_airflow_test where source_n = 'csv') ma 
